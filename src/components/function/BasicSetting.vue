...
<template>
  <div
    class="iw-contextmenu__item flex items-center justify-between w-full px-4"
  >
    <div class="left-box flex items-center">
      <i
        :class="`${
          props.layoutConf.icon ? props.layoutConf.icon : iconSvg.TEXT
        } text-lg mr-1`"
      ...
      />
      <input
        :value="props.layoutConf.title"
        class="iw-input iw-input-sm w-28"
        type="text"
        :placeholder="$t('layout.title.rename')"
        @change="renameLayoutTitle"
        @click="(e) => handleToggle(e, false)"
        @blur="(e) => handleToggle(e, true)"
      />
      <i
        class="octicon-pencil-24 icon-edit pointer-events-none"
    ...
    <div class="rightbox">
      <i
        ...
        "
        :class="[`${iconSvg.RESET}`, 'text-lg cursor-pointer mr-2']"
        :title="$t('layout.reset.title')"
      />
      <i
        @click="copyLayout"
        :class="[`${iconSvg.COPY}`, 'text-lg cursor-pointer mr-2']"
        :title="$t('layout.copy.title')"
      />
      <i
        v-if="layoutLength > 1"
        @click="
        ...
        "
        :class="[`${iconSvg.TRASH}`, 'text-lg cursor-pointer text-[#dc2626]']"
    ...
    <MenuComp ref="confirmResetLayoutCompRef" class="p-2">
      <div
        class="iw-contextmenu__item flex items-center justify-between w-full mb-2"
      >
      ...
      <div class="iw-contextmenu__item flex justify-end w-full pl-2 pr-2">
        <button
          class="iw-btn iw-btn-primary iw-btn-xs text-white"
          @click="resetLayout"
        >
    ...
    <MenuComp ref="confirmDeleteLayoutCompRef" class="p-2">
      ...
      <div class="iw-contextmenu__item flex justify-end w-full pl-2 pr-2">
        <button
          class="iw-btn iw-btn-error iw-btn-xs text-white"
          @click="deleteLayout"
        >
